...
for e in range(10):
    ...
    epoch_list.append(e)
plt.plot(epoch_list, loss_train, label='train_loss')
plt.plot(epoch_list, loss_test, label='test_loss')
...
def decode(code):
    ...
    for chunk in code.split(' '):
        num = ds.encode_morse('ˆ ' + chunk + ' $').unsqueeze(1)
        pred = model(num.cuda(), maxlen=2)
        pred = pred[1:].view(-1, pred_dim).argmax(-1)
        out = out + ds.decode_alpha(pred.cpu())[::-1]
    return out
